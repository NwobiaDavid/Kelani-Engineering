...
const SpotlightCard = ({ description, link, accentColor, index, subtext }: SpotlightCardProps) => {
  ...
  return (
    ...
    >
      <video ref={videoRef} loop autoPlay muted className="-z-10 absolute top-0 right-0 left-0 bottom-0 w-full h-full object-cover" src="/assets/videos/subsidiary-card-bg.mp4"></video>
      ...
      ></motion.div>
      <motion.div animate={{ opacity: hovered ? 1 : 0, transition: { duration: hovered ? 0.4 : 0.2 } }} className="w-full h-full inset-0 absolute flex flex-col justify-end py-[40px] md:py-[48px] px-[27px] md:px-[40px] z-10"><p className="text-white museo-sans text-[13px] md:text-[16px] h-[95px]">{subtext}</p></motion.div>
