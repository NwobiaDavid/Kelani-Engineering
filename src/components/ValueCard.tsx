    ...
    return (
        <motion.div ref={valueCardRef} className="w-full relative">
            <motion.div style={{ opacity: progress }} className="pt-[30px] md:pt-[64px] pb-[20px] md:pb-[40px] px-[8%] relative z-20">
                ...
                <p className="w-[85%] md:w-[50%] text-[12.8px] lg:text-[20px] museo-sans">{paragraph}</p>
            ...
            <motion.div style={{ width: valueCardWidth }} className="overflow-hidden w-full h-full absolute top-0 left-0">
                ...
                <motion.img ref={imageRef} src={image} style={{ right: valueCardImageRightPosition }} className="h-[50%] md:min-h-full md:h-full min-w-fit absolute bottom-0 md:top-[13px] z-10 right-0" />
            </motion.div>
        </motion.div>
    );
}
