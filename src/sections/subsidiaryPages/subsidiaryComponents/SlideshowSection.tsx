          ...
          <div className=' flex justify-center items-center '>
            <div className='flex lg:w-[70%] w-full lg:flex-row flex-col items-center mb-10 justify-center lg:justify-around '>
              ...
              <div className='flex mt-3 lg:mt-0 relative items-center justify-center lg:justify-between lg:w-[8%] md:w-[20%] w-[30%] '>
                ...
                <span className='lg:hidden p-4 absolute  lg:bottom-3 flex'>{current}/{count}</span>
              ...
              {header.map((item, index) => (
                ...
                >
                  ...
                  <motion.div
                    ...
                    className={`${current === index + 1 ? 'w-full relative flex items-center justify-center ' : 'w-full hidden'}`}
                  >
                    <img
                  ...
                  <CarouselItem className=' ' key={index} ref={ref}>
                    ...
                    >
                      <Card className='lg:w-[75%] w-full border-0'>
                        <CardContent className={item.classname + 'lg:flex-row p-4 px-6 flex-col flex w-full '}>

                          <motion.div initial={{ x: inView ? '50%' : 0, opacity: inView ? 0 : 0 }} animate={{ x: inView ? 0 : '50%', opacity: inView ? 1 : 0 }}>
                            <img src={item.img} alt='' />
                          </motion.div>

                          <motion.div
                            initial={{ x: '-50%', opacity: 0 }} animate={{ x: inView ? 0 : '-50%', opacity: 1 }}
                          ...
                          >
                            <ul className='grid text-sm lg:text-sm  w-full gap-3 lg:gap-y-0 gap-y-5 px-1 pt-10 lg:p-4 lg:grid-rows-3 md:grid-cols-2 grid-cols-1 lg:grid-cols-2'>
                              <li className='w-full flex  '>
                                <FaRegCircle className='mr-2 lg:text-[15px] mt-2' /> {item.point1}
                              </li>
                              <li className='w-full flex '>
                                <FaRegCircle className='mr-2 lg:text-[20px] mt-2' />{item.point2}
                              </li>
                              <li className='w-full flex '>
                                <FaRegCircle className='mr-2 lg:text-[20px] mt-2' /> {item.point3}
                              </li>
                              <li className='w-full flex '>
                                <FaRegCircle className='mr-2 lg:text-[20px] mt-2' /> {item.point4}
                              </li>
                              <li className='w-full flex '>
                                <FaRegCircle className='mr-2 lg:text-[20px] mt-2' /> {item.point5}
                              </li>
                              <li className='w-full flex '>
                                <FaRegCircle className='mr-2 lg:text-[20px] mt-2' /> {item.point6}
                              </li>
                            </ul>
                          </motion.div>
                        </CardContent>
